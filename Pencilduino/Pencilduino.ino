...
{ String B = "." ;
  ...
  if (command) {
    ...
    String param = inputbuffer;
    char * buff = const_cast<char*> (inputbuffer.c_str());
    short index = 0;
    char* instruction = strtok(buff, "_");
    while (index < 4) {
      if (commands[index] == instruction) {
        break;
      } else {
        index++;
      }
    ...
    switch (index) {
      case 0:
        Serial.println("Dot order");
        if (dot(const_cast<char*> (param.c_str()))) {
        ...
        break;
      case 1:
        Serial.println("Fill order");
        if (fill(const_cast<char*> (param.c_str()))) {
          Serial.println("Fill Done");
          wifi.write("Done\n");
        } else {
          Serial.println("Fill error");
          wifi.write("Error\n");
        }
        break;
      case 2:
        Serial.println("DiagonalD order");
        if (diagonal(const_cast<char*> (param.c_str()), 1)) {
          Serial.println("DiagonalD Done");
          wifi.write("Done\n");
        } else {
          Serial.println("DiagonalD error");
          wifi.write("Error\n");
        }
        break;
      case 3:
        ...
        if (diagonal(const_cast<char*> (param.c_str()), 1)) {
          ...
          Serial.println("DiagonalI Error");
          wifi.write("Error\n");
        }
        break;
      default:
        Serial.println("Invalid order case");
        wifi.write("Invalid Command\n");
        break;
    }
    Serial.println("Done");
    inputbuffer = "";
